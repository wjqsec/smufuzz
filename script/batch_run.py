...
for proj in smm_fuzz_projs:
    ...
    shutil.copyfile(ovmf_bin, os.path.join(proj[0], "OVMF_CODE.fd.backup"))
    shutil.copyfile(ovmf_vars, os.path.join(proj[0], "OVMF_VARS.fd.backup"))
    compose_command = ["python3", compose_bin, os.path.join(proj[0], proj[1]), os.path.join(proj[0], "OVMF_CODE.fd.backup")]
...
while True:
    while avaliable_cpus != 0 and len(waiting_jobs) != 0:
        ...
        avaliable_cpus -= 1
        shutil.copyfile(os.path.join(proj[0], "OVMF_CODE.fd.backup"), os.path.join(proj[0], "OVMF_CODE.fd"))
        shutil.copyfile(os.path.join(proj[0], "OVMF_VARS.fd.backup"), os.path.join(proj[0], "OVMF_VARS.fd"))
        ...
        fuzz_command = [fuzz_bin, "--proj",smm_fuzz_proj_dir, "--tag" , tag, "fuzz","--fuzz-time",fuzz_run_time,"--init-phase-timeout-time","3s"]
